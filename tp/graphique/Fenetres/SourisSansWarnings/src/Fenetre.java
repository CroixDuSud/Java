...
// Constructeur : 
// --------------------------------------------------------------------------
// Il est deconseille d'utiliser une methode "overridable" dans le 
// constructeur, de peur qu'elle soit surchargee dans une classe derivee. 
// En effet, le constructeur ci dessous est appelle des le demarrage du 
// constructeur de la classe derivee (super()).
// --------------------------------------------------------------------------
// L'usage de this dans le constructeur est egalement deconseille, car 
// l'objet peut ne pas etre entierement initialise.    
// --------------------------------------------------------------------------
...
// Constructeur :
// --------------------------------------------------------------------------
    public final void prepareFenetre()   // ou private sans final
    {
    ...
    public void mouseWheelMoved(MouseWheelEvent e)
    {
        if (getTitle().compareTo("Deplacement vertical") == 0)
        {
            setLocation(getLocation().x,
                getLocation().y + (e.getWheelRotation()
                * e.getScrollAmount()));
        }
        else
        {
            setLocation(getLocation().x + (e.getWheelRotation()
                * e.getScrollAmount()),
                getLocation().y);
        }
    }
